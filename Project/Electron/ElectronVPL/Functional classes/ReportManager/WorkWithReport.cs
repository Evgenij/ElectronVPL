...
namespace ElectronVPL
{
    class WorkWithReport : ReportManager
    {
        ...
        {
            pathDocument = AppDomain.CurrentDomain.BaseDirectory + "fileExample.docx";
        ...
        {
            pathDocument = AppDomain.CurrentDomain.BaseDirectory +
                surname + "_" +
                name + "_Lab" +
        ...
        {
            System.Diagnostics.Process.Start(GetPathLab(1));
        }

        private string GetPathLab(int numLab) 
        {
            if (numLab != 0 && numLab <= 5)
            {
                return pathLab = AppDomain.CurrentDomain.BaseDirectory + @"Labs\Лабораторная работа №" +numLab.ToString() + ".docx";
            }
            else 
            {
                return "none";
