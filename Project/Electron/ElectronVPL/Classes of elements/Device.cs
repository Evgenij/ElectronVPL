    ...
    class Device : IDisposable, IVisualization
    {
        ...
        private void InitializationPlugs(Device device)
        {
            if (device is MeterDevice)
            {
                ...
                {
                    SetPositionsPlugs(GlobalData.WorkForm, 20, 49);
                }
                else
                {
                    SetPositionsPlugs(GlobalData.WorkForm, 18, 90);
                }
            }
            else if (device is ResistanceDevice)
            {
                ...
                {
                    SetPositionsPlugs(GlobalData.WorkForm, 66, 95);
                }
            ...
            {
                SetPositionsPlugs(GlobalData.WorkForm, 180, 209);
            }
            else if (device is Capacitor)
            {
                SetPositionsPlugs(GlobalData.WorkForm, 65, 94);
            }
            else if (device is Lamp)
            {
                SetPositionsPlugs(GlobalData.WorkForm);
            }
